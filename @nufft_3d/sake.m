...
opts.maxit = 5; % maximum no. iterations
...
raw =  reshape(data, obj.nRO, obj.nTR,[]);

for iter = 1:opts.maxit

    % data consistency
    tmp = obj.fNUFT(ifft3(ksp));
    tmp = reshape(tmp, obj.nRO, obj.nTR,[]);
    tmp(obj.gap+1:end,:,:) = raw(obj.gap+1:end,:,:);

    tmp = iNUFT(obj,tmp,10,opts.damp);
    ...
    %     for lo = 1:hi
    %         h = hist(W(lo:hi),sqrt(double(hi-lo)));
    %         [~,k] = max(h);
    %         if k>1; break; end
    %     end
    %     sigma = sqrt(double(median(W(lo:hi))));
    %     opts.noise = sigma / sqrt(double(matrix_density*nx*ny));
    %     fprintf('Noise std estimate: %.2e\n',opts.noise);
    % end
    % sigma = opts.noise * sqrt(double(matrix_density*nx*ny));
    % 
    ...
    F = V * diag(f) * V';
    %  zimu work on this filter, test on fully sampeld data then unfilled 
    ...
    if mod(iter,2)==1 || converged
        display(W,f,2,ksp,iter,tol,opts,norms);
    end
...
imshow(tmp,[]); xlabel('kx'); ylabel('ky'); title('kspace');

% show current image (center of z)
subplot(1,4,3); slice = ceil(size(ksp,3)/2);
tmp = sos(ifft3(ksp)); tmp = squeeze(tmp(:,:,slice));
imshow((abs((tmp))),[]); xlabel('x'); ylabel('y');colormap(gray);
