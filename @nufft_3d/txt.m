...
opts.maxit = 100; % maximum no. iterations
...
opts.sparsity = 0.01; % compressed sensing sparsity (1=off)
...
for iter = 1:opts.maxit
    ...
    % data consistency
    tmp = ifft3(ksp);
    tmp = iNUFT(obj,obj.fNUFT(tmp)-data,0,opts.damp);
    tmp = fft3(tmp);
    ksp = ksp - tmp;
...
imagesc(tmp);colormap(gray); xlabel('kx'); ylabel('ky'); title('kspace');
...
% 
% figure(); 
% slice = ceil(size(ksp,3)/2);
% tmp = ifft(ksp,[],3); tmp = squeeze(tmp(:,:,slice,:));
% imshow(sum(abs(ifft2(tmp)),3),[],"InitialMagnification",1000, Colormap=gray); xlabel('x'); ylabel('y');
% title(sprintf('iter %i',iter));
